...
class TestTC02001001(unittest.TestCase):
    ...
    def test_TC001_001(self):
        ...
        notify_bind_result = NotifyBindResult(cardNo="3104770002054731968",type=0)
        logger.info("发起issue_card请求")
        issue_card_response=HttpUtils.http_post(
            url=notify_bind_result.url(),
            json=notify_bind_result.make_payload()
        ).json()

        error_count, error_msg_list = notify_bind_result.assert_response_data(issue_card_response)
