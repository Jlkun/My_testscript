...
class TestTC02001001(unittest.TestCase):
    ...
    def test_TC001_003(self):
        """
        用例名称:
           申请开卡测试 无效卡类型&无效合作方coBrandCard为null&''或者partnerNo为null&''
        """
        issue_card = WindIssueCard(userToken=self.userToken,coBrandCard='')
        issue_card = WindIssueCard(userToken=self.userToken,coBrandCard=None)
        issue_card = WindIssueCard(userToken=self.userToken,partnerNo='')
        issue_card = WindIssueCard(userToken=self.userToken,partnerNo=None)
        issue_card = WindIssueCard(userToken=self.userToken) #
        logger.info("发起issue_card请求")
        issue_card_response=HttpUtils.http_post(
            url=issue_card.url(),
    ...
    def test_TC001_004(self):
        """
        用例名称:
           申请开卡测试 毕传参数为空userToken缺失或者coBrandCard缺失或者partnerNo缺失或者访问不存在接口名称
        """
        issue_card = WindIssueCard(userToken=self.userToken)
        logger.info("发起issue_card请求")
        issue_card_response=HttpUtils.http_post(
    ...
    def test_TC001_005(self):
        """
        用例名称:
           申请开卡测试 coBrandCard和partnerNo不配对
        """
        issue_card = WindIssueCard(userToken=self.userToken,coBrandCard='newyorktype001')
        issue_card = WindIssueCard(userToken=self.userToken,partnerNo='newyork001')
        logger.info("发起issue_card请求")
        issue_card_response=HttpUtils.http_post(
            url=issue_card.url(),
            json=issue_card.make_payload()
