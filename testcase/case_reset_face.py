...
class TestTC02010001(unittest.TestCase):
    ...
    def test_TC01011F_001(self):
        """
        用例名称:
           合作方向复旦微wind进行换卡面
        """

        Reset_Face = ResetFace(cardNo='3104830020100299977')
        logger.info("发起issue_card请求")
        Reset_Face_response=HttpUtils.http_post(
            url=Reset_Face.url(),
            json=Reset_Face.make_payload()
        ).json()
        error_count, error_msg_list = Reset_Face_response.assert_response_data(Reset_Face_response)
