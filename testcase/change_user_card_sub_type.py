...
from interface.change_user_card_sub_type import ChangeUserCardSubType
...
class TestTC02010001(unittest.TestCase):
    ...
    def test_TC01011FF_00122(self):
        """
        用例名称:
           学校更换子卡类型接口
        """

        Change_User_Card_SubType = ChangeUserCardSubType(cardNo='3104770002054731968')
        logger.info("发起issue_card请求")
        Change_User_Card_SubType_response=HttpUtils.http_post(
            url=Change_User_Card_SubType.url(),
            json=Change_User_Card_SubType.make_payload()
        ).json()
        error_count, error_msg_list = Change_User_Card_SubType_response.assert_response_data(Change_User_Card_SubType_response)
