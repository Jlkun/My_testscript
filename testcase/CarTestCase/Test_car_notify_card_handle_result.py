...
from interface.car_get_card_handle_result import CarGetCardHandleResult
from interface.car_notify_card_handle_result import CarNotifyCardHandleResult
...
class TestTC02001001111(unittest.TestCase):
    def setUp(self):
        log.setLog()
        # 同步服务器时间
    ...
    def test_TC001car_003011(self):
        """
        用例名称:
           申请开卡测试 token过期或token为null或''
        """
        car_notify_card_handle_result = CarNotifyCardHandleResult(cardNo="3104830010005064919")
        logger.info("发起car_get_card_handle_result请求")
        car_notify_card_handle_result_response=HttpUtils.http_post(
            url=car_notify_card_handle_result.url(),
            json=car_notify_card_handle_result.make_payload()
        ).json()
        error_count, error_msg_list = car_notify_card_handle_result.assert_response_data(car_notify_card_handle_result_response)
