...
class TestTC02001001(unittest.TestCase):
    ...
    def test_TC001car_002(self):
        """
        用例名称:
           申请开卡测试 token过期或token为null或''
        """
        car_get_card_handle_result = CarGetCardHandleResult(cardNo='3104830010005064919')
        logger.info("发起car_get_card_handle_result请求")
        issue_card_response=HttpUtils.http_post(
            url=car_get_card_handle_result.url(),
            json=car_get_card_handle_result.make_payload()
        ).json()
        error_count, error_msg_list = car_get_card_handle_result.assert_response_data(issue_card_response)
