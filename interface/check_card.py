...
class CheckCard:
    def __init__(self,**kwargs):
        self._user_token = kwargs.get('userToken', None)
        list=['123123123','456456456','3104770002968547777',]
        self._cardNoList = kwargs.get('cardNoList', list)
    def url(self):
        http_schema = config.get("http_option", "http_schema")
        end_point = config.get("http_option", "wind_router_point")
        return http_schema + '://' + end_point + '/wind/sptcc/1/0/check-card'

    def make_payload(self):
        # 组装req_data
        request_body = dict()
        request_body['userToken'] = self._user_token
        request_body['cardNoList'] = self._cardNoList
        return request_body
